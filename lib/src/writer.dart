...
void replaceMetadata(File track, ParserTag metadata) {
  ...
  try {
    ...
    if (ApeParser.canUserParser(reader) && metadata is ApeMetadata) {
      write = () => ApeWriter().write(track, metadata);
    } else if (MP3Parser.hasID3v2Tag(reader)) {
      write = () => Id3v4Writer().write(track, metadata as Mp3Metadata);
    } else if (MP4Parser.canUserParser(reader)) {
      write = () => Mp4Writer().write(track, metadata as Mp4Metadata);
    } else if (FlacParser.canUserParser(reader)) {
      write = () => FlacWriter().write(track, metadata as VorbisMetadata);
    } else if (RiffParser.canUserParser(reader)) {
      write = () => RiffWriter().write(track, metadata as RiffMetadata);
    } else if (MP3Parser.hasID3v1Tag(reader)) {
      write = () => ID3v1Writer().write(track, metadata as Mp3Metadata);
    }
  } finally {
